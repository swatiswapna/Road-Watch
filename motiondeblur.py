...
def kernel_psf(angle, d, size=20):
    ...
    kernel = cv2.warpAffine(kernel, A, (size, size),
                            flags=cv2.INTER_CUBIC)  # image to specific matrix conversion
    return kernel
...
def main(val):
    ...
    i = 1
    ## reading in video
    video_path = input("/Users/swatea/Developer/hackathon_srm/Y2meta.app-MUMBAI TRAFFIC | INDIA-(1080p).mp4")
